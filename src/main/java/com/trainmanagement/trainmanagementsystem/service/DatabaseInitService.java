package com.trainmanagement.trainmanagementsystem.service;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.CommandLineRunner;
import org.springframework.core.io.ClassPathResource;
import org.springframework.jdbc.core.JdbcTemplate;
import org.springframework.stereotype.Service;

import java.nio.charset.StandardCharsets;
import java.nio.file.Files;

// @Service
public class DatabaseInitService implements CommandLineRunner {

    @Autowired
    private JdbcTemplate jdbcTemplate;

    @Override
    public void run(String... args) throws Exception {
        try {
            System.out.println("Initializing database...");
            
            // Create passenger table directly with JdbcTemplate
            String createTableSQL = """
                CREATE TABLE IF NOT EXISTS `passenger` (
                    `id` BIGINT PRIMARY KEY AUTO_INCREMENT,
                    `full_name` VARCHAR(255) NOT NULL,
                    `email` VARCHAR(255) UNIQUE NOT NULL,
                    `phone` VARCHAR(20),
                    `username` VARCHAR(50) UNIQUE NOT NULL,
                    `password` VARCHAR(255) NOT NULL,
                    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
                    `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
                ) ENGINE=InnoDB
                """;
            
            jdbcTemplate.execute(createTableSQL);
            System.out.println("Passenger table created successfully");
            
            // Insert test passengers
            String insertSQL = """
                INSERT IGNORE INTO passenger (id, full_name, email, phone, username, password) VALUES
                (1, 'John Smith', 'john.smith@email.com', '+94771234567', 'john', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi'),
                (2, 'Sarah Johnson', 'sarah.j@email.com', '+94771234568', 'sarah', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi'),
                (3, 'Admin User', 'admin@train.com', '+94771234569', 'admin', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi'),
                (4, 'Test Passenger', 'test@email.com', '+94771234570', 'test', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi')
                """;
            
            jdbcTemplate.execute(insertSQL);
            System.out.println("Test passengers inserted successfully");
            
            System.out.println("Database initialization completed.");
        } catch (Exception e) {
            System.out.println("Database initialization failed: " + e.getMessage());
            e.printStackTrace();
        }
    }
}
